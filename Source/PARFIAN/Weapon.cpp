...
void AWeapon::BeginPlay()
{
	...
	CombatCollision->OnComponentBeginOverlap.AddDynamic(this, 
		&AWeapon::CombatOnOverlapBegin);
	CombatCollision->OnComponentEndOverlap.AddDynamic(this, 
		&AWeapon::CombatOnOverlapEnd);
...
void AWeapon::Equip(AMain* Char)
{
	if (Char)
	{
		...
		const USkeletalMeshSocket* RightHandSocket = Char->GetMesh()->
	...
	bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		...
		if (Enemy)
		{
			...
			{
				const USkeletalMeshSocket* WeaponSocket = SkeletalMesh->GetSocketByName(
					"WeaponSocket");
				...
				if (WeaponSocket)
				{
					...
					UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), 
						Enemy->HitParticles, GetActorLocation(), FRotator(0.f), false);
				}
			}
